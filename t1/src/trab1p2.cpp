...
float alpha = 0.9;//Transparency of the height plot
...
{ //The function is samples 
	...
	//return cos(2*pow(x,6.0/4)+pow(y, 1.0/2));
}
...
void draw() //Render the height plot (GLUT callback function)
{
	...
	glEnable(GL_BLEND); //5. Enable blending
	...
	for(float x=Xmin;x<=Xmax-dx;x+=dx) //6. Render the height plot as a set of quads
	 for(float y=Ymin;y<=Ymax-dy;y+=dy)	
	 {
		...
		glColor3f(0,0,0); //Draw the quad corresponding to the domain-grid cell as black outline
		Quad q2; // We draw this first, and fully opaque
		q2.addPoint(p1.x,p1.y,0);					
		q2.addPoint(p2.x,p2.y,0); 
		q2.addPoint(p3.x,p3.y,0); 
		q2.addPoint(p4.x,p4.y,0);
		q2.draw();
		*/

		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); // Draw next the quad filled with green and slightly transparent
		glColor4f(1-(f(x,y)/2.0 + 0.5), f(x,y)/2.0 + 0.5, 0, alpha); //Specify the color and transparency
		Quad q1; //For each vertex, first add the normal, then the vertex
...
int main(int argc,char* argv[])	//Main program
{
   ...
   glutCreateWindow("6. Using transparency"); //4. Create our window
